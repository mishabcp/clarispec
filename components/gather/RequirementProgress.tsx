  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="border-b border-white/[0.08] px-4 py-3">
        ...
        {clientName && <p className="text-[10px] text-white/40 mt-1 uppercase tracking-wider">{clientName}</p>}
        <Badge variant="outline" className="mt-2 h-5 px-2 text-[8px] uppercase font-bold tracking-[0.2em] bg-white/[0.03] border-white/[0.1] rounded-none">
      ...
      <div className="flex flex-col items-center p-4">
        <div className="relative h-24 w-24">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            ...
            <span className="text-[8px] uppercase tracking-widest text-white/20">Progress</span>
          </div>
        </div>
        <p className="mt-2 text-[9px] uppercase tracking-widest text-white/20">
          Min. {threshold}% Target
        </p>
      </div>

      {/* Areas checklist */}
      <div className="flex-1 overflow-y-auto border-t border-white/[0.08] px-4 py-3 scrollbar-thin">
        <h4 className="mb-3 text-[9px] font-bold text-white/40 uppercase tracking-[0.2em]">
          Coverage Logic
        </h4>
          ...
          {(Object.entries(areas) as [keyof RequirementAreas, boolean][]).map(([key, covered]) => {
            ...
            return (
              <div key={key} className="flex items-center gap-2 text-[11px] font-light tracking-wide">
                ...
                <span className={covered ? 'text-white' : 'text-white/30'}>
                  {label}
                </span>
              </div>
            )
      ...
      <div className="border-t border-border px-4 py-3">
        {canGenerate && !generating && (
          <div className="mb-2 flex items-center gap-2 rounded-none bg-success/10 px-3 py-1.5 animate-pulse">
            <Sparkles className="h-3 w-3 text-success" />
            <span className="text-[9px] font-bold uppercase tracking-widest text-success">Ready to generate!</span>
          </div>
        )}
        <Button
          onClick={onGenerate}
          ...
          className={cn(
            "w-full h-10 gap-2 rounded-none transition-all duration-500 font-bold text-[9px] uppercase tracking-[0.2em] shadow-2xl",
            canGenerate && !generating
        ...
        {!canGenerate && (
          <p className="mt-2 text-center text-[9px] uppercase tracking-widest text-white/20">
